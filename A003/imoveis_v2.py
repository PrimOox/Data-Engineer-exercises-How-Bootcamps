...
def get_json(url, sz, i, headersList, payload):
    ret = requests.request("GET", url.format(sz, i), data=payload, headers=headersList)
...
while len(json_data['search']['result']['listings']) > 0:
    ...
    print(f"valor i: {qtd}\t\t qtd_imoveis: {imovel_id}")
